...
request_latency = Histogram(
    "socket_request_latency_seconds", "Socket Request latency", ["event"]
)
request_gauge = Gauge(
    "socket_active_connections", "Number of active socket connections"
)
request_count_messages = Counter(
    "messages_counter", "Number of messages sent", ["count_messages"]
)
                            ...
                            for user_id in user_ids:
                                ...
                                if user_id in self.user_conn:
                                    ...
                                    self.logger.logInfoServer(
                                        f"User {user_id} not connected, skipping"
                                    )
            ...
            try:
                ...
                if check_token is None:
                    ...
                    raise ConnectionRefusedError(
                        "Token has been marked as used in blacklist!"
                    )

                payload = jwt.decode(
                    check_token["token"], os.getenv("SECRET_KEY"), algorithms=["HS256"]
                )
