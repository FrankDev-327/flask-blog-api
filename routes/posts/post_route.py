...
from controllers.posts.post_seacht_by_title_controller import (
    PostGetSearchTitleController,
)
...
def register_post_routes(api: Api):
    ...
    api.add_resource(
        PostUpdateController, "/post/<int:post_id>", endpoint="post_update"
    )
    api.add_resource(
        PostGetSearchTitleController, "/post/search", endpoint="post_title_search"
    )
    api.add_resource(
        PostGetDetailsController, "/post/<int:post_id>", endpoint="post_get_details"
    )
    api.add_resource(
        PostListComments, "/post/<int:post_id>/comments", endpoint="post_list_comments"
    )
