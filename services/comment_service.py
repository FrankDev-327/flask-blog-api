    ...
    def createComment(self, commentBody):
        ...
        try:
            ...
            self.mention_service.create_mention(
                new_comment.content, new_comment.id, commentBody["user_mentioned_ids"]
            )
            ...
            return {
                ...
                "comment": {
                    ...
                    "created_at": helper.formatting_time(
                        new_comment.created_at, "%Y-%m-%d %H:%M:%S"
                    ),
                    "updated_at": helper.formatting_time(
                        new_comment.updated_at, "%Y-%m-%d %H:%M:%S"
                    ),
                },
            }, 201
                ...
                "children": [
                    {
                        ...
                        "created_at": helper.formatting_time(
                            child.created_at, "%Y-%m-%d %H:%M:%S"
                        ),
                        "updated_at": helper.formatting_time(
                            child.updated_at, "%Y-%m-%d %H:%M:%S"
                        ),
                    }
    ...
    def updateComment(self, comment_id, commentBody):
        ...
        try:
            update_stmt = (
                update(CommentModel)
                .where(CommentModel.id == comment_id)
                .values(content=commentBody["content"])
                .returning(CommentModel)
            )
            ...
            self.mention_service.create_mention(
                updated_comment.content, comment_id, commentBody["user_mentioned_ids"]
            )
            return {
                ...
                "comment": {
                    ...
                    "created_at": helper.formatting_time(
                        updated_comment.created_at, "%Y-%m-%d %H:%M:%S"
                    ),
                    "updated_at": helper.formatting_time(
                        updated_comment.updated_at, "%Y-%m-%d %H:%M:%S"
                    ),
                },
            }, 200
        except Exception as e:
