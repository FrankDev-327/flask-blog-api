...
class PrivateMessageService:
    ...
    def save_private_message(self, messageBody):
        try:
            stmt = insert(PrivateMessageModel).values(
    ...
    def get_private_message_by_id(self, sender_id, receiver_id, page, per_page):
        try:
            stmt = (
                ...
                .where(PrivateMessageModel.sender_id == sender_id)
            ...
            total_messages = db.session.execute(
                select(db.func.count()).select_from(PrivateMessageModel)
            ).scalar_one()
            ...
            messages_list = [
                {
                    ...
                    "created_at": self.helper.formatting_time(
                        message.created_message_at, "%Y-%m-%d %H:%M:%S"
                    ),
                    "updated_at": self.helper.formatting_time(
                        message.updated_message_at, "%Y-%m-%d %H:%M:%S"
                    ),
                }
            ...
            return {"message": f"Error retrieving private messages: {str(e)}"}, 500.0
