...
class InterestingService:
            ...
            return {
                ...
                "interesting": {
                    ...
                    "created_at": self.helper.formatting_time(
                        new_interest.created_at, "%Y-%m-%d %H:%M:%S"
                    ),
                    "updated_at": self.helper.formatting_time(
                        new_interest.updated_at, "%Y-%m-%d %H:%M:%S"
                    ),
                },
            }, 201
    ...
    def listingUsersSameInterest(self, current_user_id):
        try:
            subs_time = self.helper.formatting_time(
                "", "%Y-%m-%d %H:%M:%S", "subs_time"
            )
            ...
            stmt = (
                ...
                .where(
                    InterestingModel.interest_name.in_(
                        [
                            interest["interest_name"]
                            for interest in current_user_interests_stmt
                        ]
                    )
                )
            ...
            result = db.session.execute(stmt).all()
            for row in result:
                user_dict = {"user_id": row.id, "nick_name": row.nick_name}
